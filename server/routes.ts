...
import { resellers, resellerSales } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/products", async (req, res) => {
    try {
      ...
      const requestData = {
        ...req.body,
        entryDate: req.body.entryDate ? new Date(req.body.entryDate) : null
      };
  ...
  app.post("/api/stock-control/sessions", async (req, res) => {
    try {
      const sessionData = insertStockControlSessionSchema.parse(req.body);
  ...
  // =======================
  // RESELLERS ROUTES (ELIMINADOS DUPLICADOS)
  // =======================
