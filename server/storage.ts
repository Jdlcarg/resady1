...
class DrizzleStorage implements IStorage {
  ...
  async createStockControlSession(session: InsertStockControlSession): Promise<StockControlSession> {
    const [created] = await db.insert(stockControlSessions).values(session).returning();
    return created;
  }
